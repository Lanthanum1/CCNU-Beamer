% A Beamer template for Central China Normal University 

\documentclass[aspectratio=169]{beamer}
\usepackage{ctex, hyperref}
\usepackage[T1]{fontenc}

% other packages
\usepackage{latexsym,amsmath,amssymb,xcolor,multicol,booktabs,calligra}
\usepackage{graphicx,pstricks,listings,stackengine,subcaption}
\usepackage{CCNU}
\usepackage{xeCJK}
% \usepackage{mathptmx}

% \usefonttheme[onlymath]{serif}
\usefonttheme{serif}
\setmainfont{Times New Roman}
\setCJKmainfont[AutoFakeBold=true]{FandolKai}

\setlength{\parskip}{0.5em}    % 设置段落间距为 1 em
\setlength{\parsep}{0.2em}     % 条目内部段落的间距
\setlength{\topsep}{0.2em}     % 列表与上下文之间的距离
\setlength{\itemsep}{0.2em}    % 列表项之间的间距
% \setlength{\partopsep}{0.2em}  % 列表顶部和底部的附加间距


\author{韩子坚}
\title{MATHSENSEI: A Tool-Augmented Large Language Model for Mathematical Reasoning}    %标题
\subtitle{实验分析}    %副标题
\institute{华中师范大学计算机学院}    %机构
\date{\today}     %时间


% defs
\def\cmd#1{\texttt{\color{red}\footnotesize $\backslash$#1}}
\def\env#1{\texttt{\color{blue}\footnotesize #1}}
\definecolor{deepblue}{rgb}{0,0,0.5}
\definecolor{deepred}{rgb}{0.6,0,0}
\definecolor{deepgreen}{rgb}{0,0.5,0}
\definecolor{halfgray}{gray}{0.55}

\lstset{
  basicstyle=\ttfamily\small,
    keywordstyle=\bfseries\color{deepblue},
    emphstyle=\ttfamily\color{deepred},    % Custom highlighting style
    stringstyle=\color{deepgreen},
    numbers=left,
    numberstyle=\small\color{halfgray},
    rulesepcolor=\color{red!20!green!20!blue!20},
    frame=shadowbox,
    showstringspaces=false,
    breaklines=true,
}


\begin{document}

% \kaishu %楷书

%homepage
\begin{frame}
  \titlepage
  \begin{figure}[htpb]
    \begin{center}
      \includegraphics[width=0.1\linewidth]{pic/logo/CCNU.jpg}
    \end{center}
  \end{figure}
\end{frame}

%content
\begin{frame}{Content}
  \tableofcontents[sectionstyle=show,subsectionstyle=show/shaded/hide,subsubsectionstyle=show/shaded/hide]
\end{frame}

% 定义 sectionfiles 命令
\newcommand{\sectionfiles}[1]{%
  \edef\chapFile{#1.tex} % 构造文件名
  \IfFileExists{\chapFile}{%
    \input{\chapFile} % 如果文件存在，则包含该文件
  }{%
    \typeout{Unknown section specified: #1}% 当 #1 对应的文件不存在时输出警告
  }
}

% 检查是否定义了 \compileSection，仅包括对应部分
\ifdefined\compileSection
  \sectionfiles{\compileSection}
\else
  % 如果未定义 \compileSection，则包括所有部分
  \input{0.tex}
  \input{1.tex}
  % \input{2.tex}
  % \input{3.tex}
\fi


\begin{frame}
  \begin{center}
    {\Huge Thank you!}
  \end{center}
\end{frame}
\end{document}

